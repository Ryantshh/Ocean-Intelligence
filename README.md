...
| Relative dates | Resolved against `ai_platform/backend/clock.py`'s `working_date()` (real UTC date minus a fixed 365-day `DATA_LAG`, since the dataset's real activity sits about a year behind the database's true clock) — the extraction system prompt states this date explicitly ("Today is ..., resolve every relative date against it"), so "next month" is computed, not guessed. By default (`include_future=False`) both tables also apply this as a horizon: `update_date` on or before it, so a bare query never surfaces simulated-future rows unless the user explicitly asks about upcoming records |
...
**FIXED/OPEN/ON SUBS is a date-range containment check, not "whatever the latest report says."** A vessel is FIXED today only if *some* row (not necessarily its most recently updated one) has `commercial_status = 'FIXED'` and that row's own `[open_date_start, open_date_end]` window actually contains the simulated "today" — see `vessel_current_status`'s `active_bookings` CTE. A FIXED report whose window has already elapsed no longer makes the vessel FIXED, even if it's the newest thing on file for that vessel. `ON SUBS` gets the same window-containment treatment and reports under its own raw label — the dashboard used to relabel it WATCHLIST, but the raw data only ever contains `FIXED`/`ON SUBS`/`NULL` (confirmed live: 2,312 / 19 / 8,859 rows), so the invented label was dropped in favor of the terminology actually used in the source data. Under the current simulated date this makes FIXED/ON SUBS rare — 3 FIXED, 0 ON SUBS out of ~1,010 vessels (the exact total drifts slightly session to session as simulated-"now" is real time minus one year, always moving forward) — because this dataset's fixture windows are mostly short and rarely happen to land on the one simulated date.

**Where two rows' windows overlap and disagree, the more recently updated row overrides the older one — but only for the dates it itself covers, not the older row's whole window.** Confirmed by the sponsor with a worked example: an older row reporting OPEN 17/8–19/8 and a newer row reporting FIXED 18/8–20/8 should read OPEN on the 17th (only the older row covers that date) and FIXED from the 18th onward (the newer row now covers it, and it's the more recent report) — verified live against a synthetic reproduction of exactly that example. Every row competes on recency for this, not just FIXED/ON-SUBS ones — a plain "open" row (`commercial_status` NULL) with the latest `update_date` can override an older FIXED/ON-SUBS row the same way a newer fixture overrides an older open report; this used to be restricted to `commercial_status IN ('FIXED', 'ON SUBS')` and no longer is.

**A vessel counts as "new" the day its very first row was ever added, not the day its most-recently-touched row happens to have been received.** Confirmed by the sponsor: re-reporting an already-known vessel doesn't make it "new" again. This was a real bug — `first_date_received` was read off each vessel's *latest* row (via `vessel_current_status`, which keeps only one row per vessel), but that field drifts on ~99% of a re-reported vessel's rows (confirmed live), so it reflected that row's own receipt, not the vessel's debut. `vessel_current_status.first_date_received` is now the true per-vessel `MIN(first_date_received)` across every one of that vessel's non-future rows, computed once in the view so every downstream consumer (the Change Feed's "New vessels" and the Daily Trends chart) picks it up automatically.

**Both previously-open sponsor questions are now confirmed, and the staleness threshold is no longer a placeholder.** Staleness is specifically: the vessel's *latest* row's own `open_date_end` is more than 5 days behind simulated-today — not update-recency, and not "any lapse at all" the way `open_window_lapsed` still is. And `ON SUBS` does occupy the vessel for its window the same way a firm fixture does (the display-label question was already settled separately — it stays `ON SUBS`, matching the raw data).

**Known gaps.** `order_test` still has no commercial-status column (`scripts/glue_transform.py`'s `transform_orders` never selects one), so the orders side of the change feed can only report new/amended orders, not status changes. Staleness (`is_stale`) is deliberately still computed from a vessel's single latest-updated row, not the same "search every row for one covering today" logic `dashboard_status` uses — confirmed by the sponsor as intentional, the two signals are allowed to disagree by design, not an unreconciled inconsistency.

**Correction to an earlier version of this note:** it previously claimed `ai_platform/backend/tables/orders.py` still had a `TABLE = "orders"` constant mismatched against the real (singular) `public."order"` table. Re-checked live while investigating an unrelated question — that's wrong. The file already reads `TABLE = "order_test"` (and `tables/tonnage.py` reads `TABLE = "tonnage_test"`), so the chat agent has been on the same gold-loader-fed source as this dashboard all along; the claim was stale documentation, not a live bug, and is corrected here rather than silently deleted.
